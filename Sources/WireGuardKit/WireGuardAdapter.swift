    ...
    public func startMultihop(exitConfiguration: TunnelConfiguration, entryConfiguration: TunnelConfiguration?, completionHandler: @escaping (WireGuardAdapterError?) -> Void) {
        workQueue.async {
            ...
            do {
                ...
                self.state = .started(
                    try self.startWireGuardBackend(exitWgConfig: exitWgConfig, privateAddress: privateAddress, entryWgConfig: entryWgConfig, mtu: settingsGenerator.exit.innerMTU()),
                    settingsGenerator
                )
